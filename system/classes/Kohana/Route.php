...
class Kohana_Route {
	...
	public function matches(Request $request)
	{
		...
		if ( ! preg_match($this->_route_regex, $uri, $matches))
                {
                    if('deb' === filter_input(INPUT_GET, 'debug'))
                    {
                        echo \Debug::vars("\nL:" . __FILE__ . '(' . __LINE__ . ') $uri=', $this->_route_regex);
                    }
			return FALSE;
                }
                if('deb' === filter_input(INPUT_GET, 'debug'))
                {
                    echo \Debug::vars("\nL:" . __FILE__ . '(' . __LINE__ . ') _route_regex:'.$this->_route_regex, $matches);
                    echo \Debug::vars("\nL:" . __FILE__ . '(' . __LINE__ . ') $this:', $this);
                }
